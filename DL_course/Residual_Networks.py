    ...
    * During training you might therefore see the magnitude (or norm) of the gradient for the shallower layers decrease to zero very rapidly as training proceeds as shown below: 
    ...
    - The first CONV2D has $F_1$ filters of shape (11) and a stride of (11). Its padding is \valid\. Use 0 as the seed for the random uniform initialization: `kernel_initializer = initializer(seed=0)`. 
    - The first BatchNorm is normalizing the 'channels' axis.
    - Then apply the ReLU activation function. This has no hyperparameters. 
    
    Second component of main path:
    - The second CONV2D has $F_2$ filters of shape $(ff)$ and a stride of (11). Its padding is \same\. Use 0 as the seed for the random uniform initialization: `kernel_initializer = initializer(seed=0)`.
    - The second BatchNorm is normalizing the 'channels' axis.
    - Then apply the ReLU activation function. This has no hyperparameters.
    
    Third component of main path:
    - The third CONV2D has $F_3$ filters of shape (11) and a stride of (11). Its padding is \valid\. Use 0 as the seed for the random uniform initialization: `kernel_initializer = initializer(seed=0)`. 
    - The third BatchNorm is normalizing the 'channels' axis.
    - Note that there is **no** ReLU activation function in this component. 
    
    Final step: 
    - The `X_shortcut` and the output from the 3rd layer `X` are added together.
    - **Hint**: The syntax will look something like `Add()([var1var2])`
    - Then apply the ReLU activation function. This has no hyperparameters. 
    """
    ...
    def identity_block(X f filters training=True initializer=random_uniform):
        ...
        cache = []
        # First component of main path
        X = Conv2D(filters = F1 kernel_size = 1 strides = (11) padding = 'valid' kernel_initializer = initializer(seed=0))(X)
        X = BatchNormalization(axis = 3)(X training = training) # Default axis
        X = Activation('relu')(X)
        
        ### START CODE HERE
        ## Second component of main path (≈3 lines)
        X = Conv2D(filters=F2 kernel_size = (ff) strides =(11) padding = 'same' kernel_initializer = initializer(seed=0))(X)
        X = BatchNormalization(axis =3)(X training = training)
        X = Activation('relu')(X)
    
        ## Third component of main path (≈2 lines)
        X = Conv2D(filters=F3 kernel_size = (11) strides = (11) padding = 'valid' kernel_initializer = initializer(seed=0))(X)
        X = BatchNormalization(axis =3)(X training = training) 
    ...
    * As for the previous exercise the additional `initializer` argument is required for grading purposes and it has been set by default to [glorot_uniform](https://www.tensorflow.org/api_docs/python/tf/keras/initializers/GlorotUniform)
    ...
    First component of main path:
    - The first CONV2D has $F_1$ filters of shape (11) and a stride of (ss). Its padding is \valid\. Use 0 as the `glorot_uniform` seed `kernel_initializer = initializer(seed=0)`.
    - The first BatchNorm is normalizing the 'channels' axis.
    - Then apply the ReLU activation function. This has no hyperparameters. 
    
    Second component of main path:
    - The second CONV2D has $F_2$ filters of shape (ff) and a stride of (11). Its padding is \same\.  Use 0 as the `glorot_uniform` seed `kernel_initializer = initializer(seed=0)`.
    - The second BatchNorm is normalizing the 'channels' axis.
    - Then apply the ReLU activation function. This has no hyperparameters. 
    
    Third component of main path:
    - The third CONV2D has $F_3$ filters of shape (11) and a stride of (11). Its padding is \valid\.  Use 0 as the `glorot_uniform` seed `kernel_initializer = initializer(seed=0)`.
    - The third BatchNorm is normalizing the 'channels' axis. Note that there is no ReLU activation function in this component. 
    
    Shortcut path:
    - The CONV2D has $F_3$ filters of shape (11) and a stride of (ss). Its padding is \valid\.  Use 0 as the `glorot_uniform` seed `kernel_initializer = initializer(seed=0)`.
    ...
    def convolutional_block(X f filters s = 2 training=True initializer=glorot_uniform):
        ...
        ## Second component of main path (≈3 lines)
        X = Conv2D(filters = F2 kernel_size = (ff) strides = (1 1) padding='same' kernel_initializer = initializer(seed=0))(X) 
        X = BatchNormalization(axis = 3)(X training=training) 
        X = Activation('relu')(X) 
    
        ## Third component of main path (≈2 lines)
        X = Conv2D(filters = F3 kernel_size = (11) strides = (1 1) padding='valid' kernel_initializer = initializer(seed=0))(X) 
        X = BatchNormalization(axis = 3)(X training=training)
    ...
    - Zero-padding pads the input with a pad of (33)
    - Stage 1:
        - The 2D Convolution has 64 filters of shape (77) and uses a stride of (22). 
        - BatchNorm is applied to the 'channels' axis of the input.
        - MaxPooling uses a (33) window and a (22) stride.
    - Stage 2:
        - The convolutional block uses three sets of filters of size [6464256] \f\ is 3 and \s\ is 1.
        - The 2 identity blocks use three sets of filters of size [6464256] and \f\ is 3.
    - Stage 3:
        - The convolutional block uses three sets of filters of size [128128512] \f\ is 3 and \s\ is 2.
        - The 3 identity blocks use three sets of filters of size [128128512] and \f\ is 3.
    - Stage 4:
    ...
    - The 2D Average Pooling uses a window of shape (22).
    ...
    print (\number of training examples = \ + str(X_train.shape[0]))
    print (\number of test examples = \ + str(X_test.shape[0]))
